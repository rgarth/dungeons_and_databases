...
describe('/api/traits', () => {
  ...
  describe('GET', () => {
    ...
    it('should handle URL-encoded race names', async () => {
      const mockTraits = [
        {
          id: '1',
          name: 'Breath Weapon',
      ...
      const request = new NextRequest('http://localhost:3000/api/traits?race=Half-Elf');
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle multiple query parameters', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/traits?type=active&race=Dragonborn');
      const response = await GET(request);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data).toEqual(mockTraits);
      // Should prioritize the last filter applied (in this case, race)
      expect(mockPrisma.trait.findMany).toHaveBeenCalledWith({
        where: {
          raceTraits: {
            some: {
